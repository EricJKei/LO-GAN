...
class dataloader():
    ...
    def build_loader(self):
        ...
        if self.mode == 'train':
            ...
            random.seed(0)
            random.shuffle(tr_sharp_imgs)
            random.seed(0)
            random.shuffle(tr_blur_imgs)
            train_list = (tr_blur_imgs, tr_sharp_imgs)
            
            self.tr_dataset = tf.data.Dataset.from_tensor_slices(train_list)
            self.tr_dataset = self.tr_dataset.map(self._parse, num_parallel_calls = 3).prefetch(8)
            # self.tr_dataset = self.tr_dataset.map(self._get_patch, num_parallel_calls = 2).prefetch(16)
            if self.augmentation:
                self.tr_dataset = self.tr_dataset.map(self._data_augmentation, num_parallel_calls = 3).prefetch(8)
            # self.tr_dataset = self.tr_dataset.shuffle(16)
            ...
            if self.test_with_train:
                ...
                self.val_dataset = self.val_dataset.map(self._parse, num_parallel_calls = 3).prefetch(8)
        ...
        elif self.mode == 'test' or self.mode == 'test_only':
            ...
            self.te_dataset = self.te_dataset.map(self._parse_Blur_only, num_parallel_calls = 3).prefetch(8)
            self.te_dataset = self.te_dataset.batch(self.batch_size)
    ...
    def _parse(self, image_blur, image_sharp):
        ...
        image_blur = tf.image.decode_jpeg(image_blur, channels = self.channel)
        image_sharp = tf.image.decode_jpeg(image_sharp, channels = self.channel)
